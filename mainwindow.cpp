...

{
    ...
    connect(worker, SIGNAL(valueChanged(QString)), ui->textBrowser_console, SLOT(setText(QString)));
...
void MainWindow::on_pushButton_generate_simple_doz_clicked() 
{
    ...
    float k5ml = ui->lineEdit_dosing_coeff->text().toFloat(); // mm / ml коэфф пересчёта для шприца
